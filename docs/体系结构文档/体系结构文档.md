...
| AdminService.getAllSaleMen     | 获得所有营销人员信息     |
...
| CouponService.getHotelAllCoupon   | **语法**：List<Coupon> getWebsiteAllCoupon()<br>**前置条件**：<br>**后置条件**：得到并返回所有网站优惠策略 |
| CouponService.addCoupon           | **语法**: CouponVO addCoupon(CouponVO couponVO)<br>**前置条件**:无<br>**后置条件**:添加优惠策略 |
| CouponService.deleteCoupon        | **语法**：ResponseVO deleteCoupon(int couponId)<br>**前置条件**：<br>**后置条件**：根据couponId来删除优惠策略 |
...
| AccountService.updateUserCreditByEmail | **语法**：ResponseVO updateUserCreditByEmail(String email, double credit)<br>**前置条件**：<br>**后置条件**：根据email, credit向用户添加信用值 |
...
| AdminMapper.updateHotel       | **语法**：int updateHotel(Hotel hotel)<br>**前置条件**：<br>**后置条件**：根据hotel更新酒店信息 |
| AdminMapper.manageUserInfo    | **语法**：User managerUserInfo(int id)<br>**前置条件**：<br>**后置条件**：根据id获取用户信息 |
...
| HotelMapper.selectByManagerId   | **语法**：HotelVO selectByManagerId(Integer ,manager_id)<br>**前置条件**：<br>**后置条件**：根据manager_id得到酒店信息 |
